<?php 
	...
	if ( SESSION_EXISTS ) {
		...
		if ( $post ) {
			...
			if ( count($aTickets) > 0 ) {
				$cArqHTML = $_SERVER['DOCUMENT_ROOT'].$security->base_patch.'/assets/relatorios/ticketDesempenhoReport_'.Date('Y-m-d_H-s-i').'_'.SEC_USER_NOME.'.html';
				$fp = fopen($cArqHTML, 'w');
				fwrite($fp, $cReportPDF);
				fclose($fp);
				$cArqHTML = $security->base_patch.'/assets/relatorios/ticketDesempenhoReport_'.Date('Y-m-d_H-s-i').'_'.SEC_USER_NOME.'.html';
