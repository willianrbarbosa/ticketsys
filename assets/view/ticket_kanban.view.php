...
<script type="text/javascript">
	...
	//     'mouseup': function() {
	//         clicked = false;
	//         $('.kanban-row').css('cursor', 'auto');
	//     }
	// });

	// var updateScrollPos = function(e) {
	//     $('.kanban-row').css('cursor', 'grab');
	//     $('.kanban-row').scrollLeft($('.kanban-row').scrollLeft() + (clickX - e.pageX));
	// };
</script>
	...
	<div class="kt-container  kt-container--fluid ">
		<div class="kt-subheader__main">
			...
			<div class="kt-subheader__breadcrumbs">
				...
				<a href="#/ticketkanban" class="kt-subheader__breadcrumbs-link bold">&nbsp;Kanban</a>
			</div>
		</div>
		<div class="kt-subheader__toolbar">
			<div class="kt-subheader__wrapper">
				<div class="dropdown dropdown-inline">
					...
					<div class="btn btn-sm btn-link">
						<selectize id="pasta_trabalho" name="pasta_trabalho" style="width: 280px;" config="cfgPastaTrabalho" options="aPastaTrabalho" ng-model="filter_pst_id"></selectize>
					</div>

					<a href="" class="btn btn-sm btn-default" ng-click="loadTicketsKanban()" title="Atualizar">
...
<div class="container-fluid">
    ...
    <div class="kanban-row">
        <div class="kanban-col" ng-repeat="situacao in aSituacaoKanban" ui-on-Drop="AlteraSituacaoKanban($event,$data,situacao.tickets, situacao.stt_id)">
            <div class="card">
                <div class="card-header center-justify">
                	<h5 class="card-title bold"
                		ng-class="{
                			'text-warning': situacao.stt_aprova_ticket == 'N' && situacao.stt_encerra_ticket == 'N',
                			'text-primary': situacao.stt_aprova_ticket == 'S' && situacao.stt_encerra_ticket == 'N',
                			'text-success': situacao.stt_encerra_ticket == 'S',
                		}">
                		<i class="fa" style="float: left;"
                		ng-class="{
                			'fa-clock': situacao.stt_aprova_ticket == 'N' && situacao.stt_encerra_ticket == 'N',
                			'fa-thumbs-o-up': situacao.stt_aprova_ticket == 'S' && situacao.stt_encerra_ticket == 'N',
                			'fa-check': situacao.stt_encerra_ticket == 'S',
                		}"></i>
                    	{{situacao.stt_descricao}}&nbsp;({{situacao.tickets.length}})
                    </h5>
                </div>
                <div class="card-body">

                    <div class="form-group kanban-card" ng-repeat="ticket in situacao.tickets | orderBy: 'prt_prioridade' | filter: {tkt_pst_id: filter_pst_id}"
                    	ui-draggable="true" drag="ticket" ng-show="!filter_resp_id || ticket.resp_user_id == filter_resp_id"
                    	on-drop-success="confirmaAlteraSituacao($event,$index,situacao.tickets)"

                    	ng-style="{'border-left': '7px solid ' + ticket.prt_cor, 'background-color': (ticket.tkt_encerrado == 'S' ? '#DCDCDC': '#FFF')}">
                    	<div class="row" ng-class="{'bg-gray': ticket.tkt_encerrado == 'S'}">
	                        <div class="col-xs-9">
	                        	<a href="" class="ft-black" ng-click="getTicket(ticket.tkt_id)">
		                        	<h5 class="bold">#{{ticket.tkt_id}}</h5>
		                            <p class="bold">{{ticket.tkt_titulo}}</p>
		                        </a>
		                    </div>
	                        <div class="col-xs-3">
								<img class="img-min-grid-lg col-radius" 
									title="Solicitante: {{ticket.solic_user_nome}}" 
									src="<?php echo $security->base_patch.'/assets/img/sys_images/'; ?>{{ticket.solic_user_photo}}"
									ng-if="ticket.solic_user_photo">
								<span class="kt-badge kt-badge--username kt-badge--unified-primary kt-badge--lg bold col-radius"
									title="Solicitante: {{ticket.solic_user_nome}}" 
									ng-if="!ticket.solic_user_photo">
									{{ticket.solic_user_nome.split(' ')[0].substr(0,1)}}
									{{(ticket.solic_user_nome.split(' ')[1] != 'de' ? 
										(ticket.solic_user_nome.split(' ')[1] != 'da' ? 
											ticket.solic_user_nome.split(' ')[1].substr(0,1) 
											: ticket.solic_user_nome.split(' ')[2].substr(0,1)
										) 
										: ticket.solic_user_nome.split(' ')[2].substr(0,1))}}
								</span>
								<br/>
								<img class="img-min-grid-lg col-radius" 
									title="Responsável: {{ticket.resp_user_nome}}" 
									src="<?php echo $security->base_patch.'/assets/img/sys_images/'; ?>{{ticket.resp_user_photo}}"
									ng-if="ticket.resp_user_photo">
								<span class="kt-badge kt-badge--username kt-badge--unified-primary kt-badge--lg bold col-radius"
									title="Solicitante: {{ticket.resp_user_nome}}" 
									ng-if="!ticket.resp_user_photo">
									{{ticket.resp_user_nome.split(' ')[0].substr(0,1)}}
									{{(ticket.resp_user_nome.split(' ')[1] != 'de' ? 
										(ticket.resp_user_nome.split(' ')[1] != 'da' ? 
											ticket.resp_user_nome.split(' ')[1].substr(0,1) 
											: ticket.resp_user_nome.split(' ')[2].substr(0,1)
										) 
										: ticket.resp_user_nome.split(' ')[2].substr(0,1))}}
								</span>
	                        </div>
                    	...
                    	<div class="row" ng-class="{'bg-gray': ticket.tkt_encerrado == 'S'}">
	                        <div class="col-xs-12">
		                        <p class="ft-gray-dk">{{ticket.grt_descricao}} > {{ticket.pst_descricao}}</p>
	                        	
	                        	<label class="label-prazo" ng-if="ticket.tkt_data_fim_estim && ticket.tkt_encerrado == 'N'"
	                        		ng-class="{'bg-danger3': ticket.PRAZO == 'N', 'bg-warning-dark': ticket.PRAZO == 'D', 'bg-success2': ticket.PRAZO == 'S'}"
	                        		title="{{ticket.tkt_data_ini_estim | date: 'dd/MM/yyyy'}} ás {{ticket.tkt_hora_ini_estim}} - {{ticket.tkt_data_fim_estim | date: 'dd/MM/yyyy'}} ás {{ticket.tkt_hora_fim_estim}}">
	                        		{{ticket.tkt_data_fim_estim | date: 'dd/MM/yyyy'}}
	                        	</label>

	                        	<span class="right" ng-if="ticket.qtde_comentarios > 0">&nbsp;<i class="fa fa-lg fa-comments-o"></i> <strong>{{ticket.qtde_comentarios}}</strong>&nbsp;</span>
	                        	<span class="right" ng-if="ticket.qtde_arquivos > 0">&nbsp;<i class="fa fa-lg fa-paperclip"></i> <strong>{{ticket.qtde_arquivos}}</strong>&nbsp;</span>

	                        	<label class="label-prazo right" ng-if="ticket.tkt_data_fim_estim && ticket.tkt_encerrado == 'S'"
	                        		ng-class="{'bg-danger3': ticket.dias_desvio_prazo > 0, 'bg-success2': ticket.dias_desvio_prazo <= 0}"
	                        		title="{{ticket.tkt_data_ini_estim | date: 'dd/MM/yyyy'}} ás {{ticket.tkt_hora_ini_estim}} - {{ticket.tkt_data_fim_estim | date: 'dd/MM/yyyy'}} ás {{ticket.tkt_hora_fim_estim}}">
	                        		{{ticket.tkt_data_fim_real | date: 'dd/MM/yyyy'}}
	                        	</label>

	                        </div>
	                    </div>
	                    <div class="row" ng-if="ticket.tkt_encerrado == 'N'">
	                        <div class="col-xs-12">
	                        	<a href="" class="btn btn-sm btn-default" ng-click="getTicketArquivo(ticket)" title="Anexar Arquivos" ng-if="AccessRotFilter('ticket_arquivos')">
									<i class="fa fa-paperclip"></i>
								</a>
								<a href="" class="btn btn-sm btn-default" ng-click="getTicketApontamento(ticket)" title="Apontamento manual de horas" ng-if="AccessRotFilter('ticket_apontamentos')">
									<i class="fa fa-clock"></i>
								</a>
								<a href="" class="btn btn-sm btn-default" title="Iniciar execução de atividade" 
									ng-click="novoKanbanApontamentoTicket(ticket, nTicketApontamento, false);" 
									ng-if="aUserAccess.pta_nivel >= 3 && !lDisableFields && !isUsuarioApontamentoExecucao(ticket.apontamento_pendente, aUserData.user_id)">
									<i class="fa fa-play"></i>
								</a>
								<a href="" class="btn btn-sm btn-default" title="Pausar execução de atividade" 
									ng-click="encerraKanbanApontamentoTicket(ticket, ticket.apontamento_pendente, aUserData.user_id);" 
									ng-if="aUserAccess.pta_nivel >= 3 && !lDisableFields && isUsuarioApontamentoExecucao(ticket.apontamento_pendente, aUserData.user_id)">
									<i class="fa fa-pause"></i>
								</a>
								<a href="" class="btn btn-sm btn-default" ng-click="getTicketComentario(ticket)" title="Inserir comentário" ng-if="AccessRotFilter('ticket_comentarios')">
									<i class="fa fa-comments-o"></i>
								</a>

	                        	<div class="btn" ng-repeat="apontamento in ticket.apontamento_pendente" ng-if="ticket.apontamento_pendente">
						...
						<div class="row">
	                        <div class="col-xs-12 right-justify">
                        	</div>
	                    </div>
                    </div>
								...
								<div class="form-group row">
									<div class="col-sm-6" >
										...
										<div class="input-group">
										  	...
										  	<input type="text" name="kb_tkt_data_ini_estim" id="kb_tkt_data_ini_estim"  ng-maxlength="10" maxlength="10" class="form-control my-control iptdate" ng-model="eTicket.tkt_data_ini_estim" placeholder="DD/MM/YYY" ng-disabled="true"/>
									 	</div>
										<div class="input-group">
										  	<span class="input-group-addon" style="font-size: 11px;"><i class="fa fa-lg fa-clock"></i></span>
										  	<input type="text" name="kb_tkt_hora_ini_estim" id="kb_tkt_hora_ini_estim"  ng-maxlength="10" maxlength="10" class="form-control my-control ipthora" ng-model="eTicket.tkt_hora_ini_estim" placeholder="HH:mm" ng-disabled="true"/>
									 	</div>
									</div>
									<div class="col-sm-6" >
										...
										<div class="input-group">
									  		<span class="input-group-addon" style="font-size: 11px;"><i class="fa fa-lg fa-calendar"></i></span>
									  		<input type="text" name="kb_tkt_data_fim_estim" id="kb_tkt_data_fim_estim"  ng-maxlength="10" maxlength="10" class="form-control my-control iptdate" ng-model="eTicket.tkt_data_fim_estim" placeholder="DD/MM/YYY" ng-disabled="true"/>
									 	</div>
										<div class="input-group">
									  		<span class="input-group-addon" style="font-size: 11px;"><i class="fa fa-lg fa-clock"></i></span>
									  		<input type="text" name="kb_tkt_hora_fim_estim" id="kb_tkt_hora_fim_estim"  ng-maxlength="10" maxlength="10" class="form-control my-control ipthora" ng-model="eTicket.tkt_hora_fim_estim" placeholder="HH:mm" ng-disabled="true"/>
									 	</div>
									</div>
								</div>
								<div class="form-group row">
									<div class="col-sm-6" >
										...
										<div class="input-group">
										  	<span class="input-group-addon" style="font-size: 11px;"><i class="fa fa-lg fa-calendar"></i></span>
										  	<input type="text" name="kb_tkt_data_ini_real" id="kb_tkt_data_ini_real"  ng-maxlength="10" maxlength="10" class="form-control my-control iptdate" ng-model="eTicket.tkt_data_ini_real" placeholder="DD/MM/YYY" readonly disabled  ng-disabled="true"/>
									 	</div>
										<div class="input-group">
										  	<span class="input-group-addon" style="font-size: 11px;"><i class="fa fa-lg fa-clock"></i></span>
										  	<input type="text" name="kb_tkt_hora_ini_real" id="kb_tkt_hora_ini_real"  ng-maxlength="10" maxlength="10" class="form-control my-control ipthora" ng-model="eTicket.tkt_hora_ini_real" placeholder="HH:mm" readonly disabled  ng-disabled="true"/>
									 	</div>
									</div>
									<div class="col-sm-6" >
										...
										<div class="input-group">
									  		<span class="input-group-addon" style="font-size: 11px;"><i class="fa fa-lg fa-calendar"></i></span>
									  		<input type="text" name="kb_tkt_data_fim_real" id="kb_tkt_data_fim_real"  ng-maxlength="10" maxlength="10" class="form-control my-control iptdate" ng-model="eTicket.tkt_data_fim_real" placeholder="DD/MM/YYY" readonly disabled  ng-disabled="true"/>
									 	</div>
										<div class="input-group">
									  		<span class="input-group-addon" style="font-size: 11px;"><i class="fa fa-lg fa-clock"></i></span>
									  		<input type="text" name="kb_tkt_hora_fim_real" id="kb_tkt_hora_fim_real"  ng-maxlength="10" maxlength="10" class="form-control my-control ipthora" ng-model="eTicket.tkt_hora_fim_real" placeholder="HH:mm" readonly disabled  ng-disabled="true"/>
									 	</div>
									</div>
								</div>
								...
								<div class="row">
									<div class="col-sm-6" >
										...
										<div class="input-group">
											...
											<input type="text" name="kb_tkt_total_hora_estim" id="kb_tkt_total_hora_estim"  ng-maxlength="12" maxlength="12" class="form-control my-control iptcur" ng-model="eTicket.tkt_total_hora_estim"  ng-disabled="true"/>
										</div>
									</div>
									<div class="col-sm-6" >
										<label class="bold">Real</label><br/>
									  	<input type="text" name="kb_tkt_total_hora_real" id="kb_tkt_total_hora_real"  ng-maxlength="12" maxlength="12" class="form-control my-control iptcur" ng-model="eTicket.tkt_total_hora_real" disabled readonly  ng-disabled="true"/>
									</div>
								</div>
							</div>
