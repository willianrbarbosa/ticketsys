<?php
	...
	class Security extends Session {
		...
		* @return bool
		*/
	    public function insert_data($ssTableName,$asFields,$amValues,&$nInsertID,&$cReturnMsg) {	
		...
		* @return bool
		*/
	    public function update_data($ssTableName,$asFields,$amValues,$aUpdKeys, $aUpdValues,&$cReturnMsg) {	
			if( (Empty($ssTableName)) OR (Empty($asFields)) OR (Empty($amValues)) ) {
				return false;
			} elseif ( count($asFields) <> count($amValues) ) {
				$cReturnMsg = 'Qtde. de campos da tabela \'asFields\' difere dos valores para inserção \'amValues\'.';
		...
		* @return bool
		*/
	    public function erase_data($ssTableName,$aUpdKeys, $aUpdValues,&$cReturnMsg) {	
